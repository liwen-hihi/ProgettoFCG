    ...
    public:
        ...
        Mesh (const std::string& filename) {
            ...
            for (unsigned int i = 0; i < fnum; ++i) {
                ...
                // Leggiamo tutti gli indici della faccia corrente
                std::vector<unsigned int> face_indices(vcount);
                for (unsigned int j = 0; j < vcount; ++j) {
                    if (!(file >> face_indices[j])) {
                        fprintf (stderr, "Error: Failed to read vertex index %u at face %u\n", j, i);
                        exit (1);
                    }
                }

                // Triangolazione "a ventaglio" (Triangle Fan) a partire dal primo vertice
                for (unsigned int j = 1; j < vcount - 1; ++j) {
                    glm::uvec3 triangle;
                    triangle[0] = face_indices[0];
                    triangle[1] = face_indices[j];
                    triangle[2] = face_indices[j + 1];
                    triangles.push_back(triangle);
                }
            }
